...
\begin{figure}[H]
    ...
    \subfloat[Speedup $S(n)$.\label{fig:distributed-speedup}]{%
        \scaffoldbox{0.42}{$S(n)=T_2/T_n$, broadcast vs.\ partitioned, ideal-linear $S(n)=n/2$ reference}}%
    \hfill
    \subfloat[Parallel efficiency $E(n)$.\label{fig:distributed-efficiency}]{%
        \scaffoldbox{0.42}{$E(n)=(2/n)\,S(n)$, same two series, ideal $E(n)=1$ reference}}%
...
\begin{figure}[H]
    ...
    \scaffoldbox{0.8}{Wall-clock vs.\ worker count; PostGIS \& DuckDB baselines as horizontal lines; crossover marked}
...
\begin{figure}[H]
    ...
    \subfloat[Wall-clock time by phase.\label{fig:distributed-phase-time}]{%
        \scaffoldbox{0.42}{Stacked time: executor-read / shuffle / driver-collection, vs.\ worker count}}%
    \hfill
    \subfloat[Shuffle bytes.\label{fig:distributed-shuffle-bytes}]{%
        \scaffoldbox{0.42}{Shuffle bytes vs.\ worker count (same strategy/series)}}%
...
\begin{figure}[H]
    ...
    \scaffoldbox{0.75}{Cost vs.\ time scatter, one point per worker count, broadcast vs.\ partitioned; non-dominated frontier traced}
