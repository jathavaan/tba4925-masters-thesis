...
\begin{figure}[H]
    ...
    \scaffoldbox{0.9}{Bytes grid: pattern (rows) $\times$ tier (cols), grouped bars per config, log $y$, CI whiskers; received vs.\ sent; Shapefile network $\approx 0$ (local read) annotated}
...
\begin{figure}[H]
    ...
    \scaffoldbox{0.7}{(Optional) Bytes vs.\ wall-clock time scatter, one point per config $\times$ pattern $\times$ tier, log--log}
...
\begin{figure}[H]
    ...
    \scaffoldbox{0.9}{Wall-clock grid: pattern (rows) $\times$ tier (cols), bars = minimum estimator, log $y$, bootstrap CI whiskers}
...
\begin{figure}[H]
    ...
    \scaffoldbox{0.9}{Stacked cost bars per config, faceted by pattern; four terms: compute / storage / operations / egress}
