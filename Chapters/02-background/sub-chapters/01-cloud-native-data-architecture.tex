...
% Academic rewrite
Cloud-native data architecture supplies the setting for the formats and engines this thesis evaluates, built on a single organizing move that the section takes apart in stages. That move places data in one layer and processing in another, and the section opens by establishing this decoupled model and the pressure it puts on how a format is laid out on disk. It then follows each separated layer in turn: how the storage layer, reduced to keyed objects, is reached across a network, and how the compute layer is organized as either a single process or a coordinated set of workers. A cost model that meters each of these resources separately, and that governs the choice between those two compute models, closes the section. The account starts from the separation itself.
...
\begin{figure}[H]
    ...
    \subfloat[Non-distributed compute.\label{fig:non-distributed-compute}]{%
        \includegraphics[valign=t, width=0.45\textwidth]{Chapters/02-background/sub-chapters/figures/02-non-distributed-compute.pdf}%
    }
    \hfill
    \subfloat[Distributed compute.\label{fig:distributed-compute}]{%
        \includegraphics[valign=t, width=0.45\textwidth]{Chapters/02-background/sub-chapters/figures/02-distributed-compute.pdf}%
    }
